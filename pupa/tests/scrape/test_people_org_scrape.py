...
def test_person_add_membership_org():
    ...
    p.add_membership(
        o, role="member", start_date="2007", end_date=datetime.date(2015, 5, 8)
    )
...
def test_legislator_related_chamber_district():
    ...
    assert get_pseudo_id(leg._related[0].post_id) == {
        "organization__classification": "upper",
        "label": "1",
    }
...
def test_person_add_membership_name():
    ...
    p.add_membership(
        "Academy of Motion Picture Arts and Sciences", role="winner", start_date="2016"
    )
    p._related[0].validate()
