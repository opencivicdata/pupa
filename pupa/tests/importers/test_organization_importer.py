...
def test_deduplication_similar_but_different():
    ...
    # has a parent
    o4 = ScrapeOrganization(
        "United Nations", classification="international", parent_id=o1._id
    )
...
def test_deduplication_prevents_identical():
    ...
    org2 = ScrapeOrganization(
        "United Nations", classification="international", founding_date="1945"
    )
...
def test_pseudo_ids():
    ...
    senate = Organization.objects.create(
        id="2", name="Senate", classification="upper", jurisdiction_id="jid1"
    )
    house = Organization.objects.create(
        id="3", name="House", classification="lower", jurisdiction_id="jid1"
    )
    un = Organization.objects.create(
        id="4",
        name="United Nations",
        classification="international",
    ...
    with pytest.raises(UnresolvedIdError):
        oi1.resolve_json_id(
            '~{"classification":"international", "name":"United Nations"}'
        )
