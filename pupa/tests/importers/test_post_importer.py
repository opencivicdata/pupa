...
def test_resolve_special_json_id():
    ...
    Post.objects.create(
        id="pres", label="executive", role="President", organization_id="us"
    )
    Post.objects.create(
        id="vpres", label="vice-executive", role="Vice President", organization_id="us"
    )
    Post.objects.create(
        id="gov", label="executive", role="Governor", organization_id="nc"
    )

    oi = OrganizationImporter("")
    assert PostImporter("us", oi).resolve_json_id('~{"label": "executive"}') == "pres"
    assert (
        PostImporter("us", oi).resolve_json_id('~{"label": "vice-executive"}')
        == "vpres"
    )
