...
def items_differ(jsonitems, dbitems, subfield_dict):
    ...
    # if we get here, jsonitems has to be empty because we asserted that
    # the length was the same and we found a match for each thing in dbitems,
    # here's a safety check just in case
...
class BaseImporter(object):
    ...
    def resolve_json_id(self, json_id, allow_no_match=False):
        ...
        if json_id.startswith("~"):
            # keep caches of all the pseudo-ids to avoid doing 1000s of lookups
            # during import
    ...
    def _prepare_imports(self, dicts):
        ...
        also serves as a good place to override if anything special has to be
        done to the order of the import stream (see OrganizationImporter)
        """
        # hash(json): id
        seen_hashes = {}
    ...
    def import_data(self, data_items):
        ...
        # all objects are loaded, a perfect time to do
        # inter-object resolution and other tasks
        self.postimport()
